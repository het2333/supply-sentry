...
const payableStatusLabels: Record<string, string> = { received: "已收票", matched: "匹配通过", exact_match: "完全匹配", within_tolerance: "容差内匹配", approval_required: "待审批", severe_exception: "严重异常", amount_variance: "金额差异待核对", awaiting_receipt: "待收货单", amounts_aligned: "金额一致，待三单匹配", open: "待处理", pending: "待处理", waiting_approval: "待审批", assigned: "处理中", rejected: "已拒绝", approved: "已批准" };
const statusName = (value: unknown) => {
  const raw = text(value, "待处理");

  return payableStatusLabels[raw] ?? raw;
};
function PayableStatus({ value }: { value: unknown }) {
  return <span data-preserve-language={text(value, "") && !payableStatusLabels[text(value, "")] ? true : undefined}>{statusName(value)}</span>;
}
...
function FactRow({ label, value, emphasized, tone, last, preserve = false }: { label: string; value: string; preserve?: boolean; emphasized?: boolean; tone?: ReturnType<typeof statusTone>; last?: boolean }) { const color = tone === "red" || emphasized ? "text-red-600" : tone === "green" ? "text-emerald-600" : "text-slate-800"; return <div className={`flex items-center justify-between gap-4 px-4 py-3 text-sm ${last ? "" : "border-b border-slate-100"}`}><span className="text-slate-500">{label}</span><span data-preserve-language={preserve ? true : undefined} className={`text-right font-medium ${color}`}>{value}</span></div>; }
...
function MiniFact({ label, value, preserve = false }: { label: string; value: unknown; preserve?: boolean }) { return <div className="min-w-0"><div className="text-[10px] text-[#8795a9]">{label}</div><div data-preserve-language={preserve && typeof value === "string" && value.trim() ? true : undefined} className="mt-1 truncate text-xs font-medium text-[#26364f]">{text(value, "未提供")}</div></div>; }
