...
function Preview({ title, icon: Icon, items, empty }: { title: string; icon: typeof PackageCheck; items: Record<string, unknown>[]; empty: string }) {
  return <div className="rounded-xl border border-slate-200 p-3"><div className="flex items-center gap-2 text-xs font-semibold text-slate-700"><Icon className="size-3.5 text-slate-400" />{title}</div>{items.length ? <div className="mt-2 space-y-2">{items.slice(0, 3).map((item, index) => <div key={String(item.id ?? index)} className="rounded-lg bg-slate-50 p-2 text-xs"><div className="truncate font-medium text-slate-800">{itemText(item.title ?? item.name ?? item.displayNumber ?? item.number ?? item.invoiceNumber ?? item.externalId ?? item.id)}</div><div className="mt-1 truncate text-slate-500">{itemText(item.status ?? item.supplierName ?? item.supplierId ?? item.currency)}</div></div>)}</div> : <div className="mt-2 text-xs text-slate-400">{empty}</div>}</div>;
}
...
function AttachmentPreview({ attachments }: { attachments: Attachment[] }) {
  ...
  return <div className="mt-3 space-y-3">{attachments.map((attachment, index) => {
    ...
    return <div key={attachment.id ?? `${attachment.fileName ?? "attachment"}-${index}`} className="rounded-lg border border-slate-200 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div className="min-w-0"><div className="truncate text-xs font-medium text-slate-800">{itemText(attachment.fileName, "未命名附件")}</div><div className="mt-1 text-[11px] text-slate-500">{itemText(attachment.contentType, "类型未提供")} · {formatBytes(attachment.sizeBytes)}</div></div>{url && (!link.inline || (!isImage && !isPdf)) && <a href={url} target="_blank" rel="noopener noreferrer" referrerPolicy="no-referrer" className="shrink-0 text-xs text-emerald-700 underline">安全打开</a>}</div>{url && link.inline && isImage ? <Image src={url} alt={itemText(attachment.fileName, "附件预览")} width={960} height={640} unoptimized referrerPolicy="no-referrer" className="mt-3 max-h-64 w-full rounded-lg border border-slate-100 object-contain" /> : url && link.inline && isPdf ? <iframe title={itemText(attachment.fileName, "PDF 附件")} src={url} sandbox="allow-same-origin" referrerPolicy="no-referrer" className="mt-3 h-64 w-full rounded-lg border border-slate-200" /> : <div className="mt-2 text-xs text-slate-400">{!attachment.url ? "暂无可访问 URL，无法预览。" : !url ? "附件 URL 不安全，已阻止打开。" : !link.inline ? "外部附件不会自动加载，以避免跨域泄露；请显式安全打开。" : "附件类型不支持内嵌预览，已提供安全打开链接。"}</div>}</div>;
  })}</div>;
}
